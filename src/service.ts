...
const parse = (str: string) => {
    ...
    const removeSpaces = SeparateOpsAndNums.filter((i) => i !== '');
    ...
    const result = removeSpaces.map((i) => parseFloat(i) || i === '0' ? Number(i) : i).filter(i => i !== '')
  ...
  export const sqrt = (string: string) => {
    ...
    const result = array
      .map((item: number, index) => index === array.length - 1 ? item < 0 ? 'cant sqrt negative' : Math.sqrt(item) : item)
      .join('');
